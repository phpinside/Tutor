...
import { generateInternshipCertificateUploadPdfKey, generateUploadToken } from '@/lib/qiniu'
import { QINIU_CONFIG } from '@/lib/config'

/** 老师上传自定义模板实习证明 PDF 时获取七牛上传凭证。 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (fileExt !== '.pdf') {
      return NextResponse.json({ error: '实习证明必须为 PDF 格式' }, { status: 400 })
    }

    const key = generateInternshipCertificateUploadPdfKey(teacherId, fileExt)
    ...
    console.error('生成实习证明上传凭证失败:', error)
